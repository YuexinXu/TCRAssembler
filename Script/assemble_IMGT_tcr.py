#!/usr/bin/env python3
"""
Script to assemble TRB-P2A-TRA sequences from TCR input data and IMGT templates.

This script:
1. Assembles TRB sequences from TRBV, CDR3, TRBJ, and TRBC templates
2. Assembles TRA sequences from TRAV, CDR3, TRAJ, and TRAC templates
3. Handles CDR1/CDR2 variants (IMGT vs Seq)
4. Checks for restriction sites and makes silent mutations
5. Assembles final TRB-P2A-TRA sequences
"""
...
def align_and_merge_v_cdr3(v_seq, cdr3_nt):
    ...
    Returns merged sequence with CDR3 in uppercase.
    """
    v_seq = v_seq.upper()
...
def align_and_merge_cdr3_j(v_cdr3_merged, j_seq, cdr3_nt_original):
    ...
    Returns merged sequence with J in lowercase.
            ...
            for j, (codon2_start, codon2_old, codon2_alts) in enumerate(overlapping_codons):
                ...
                for alt1 in codon1_alts:
                    for alt2 in codon2_alts:
                        # Apply both mutations (from end to start to avoid position shifts)
                        test_seq = seq
...
def remove_restriction_sites(seq):
    ...
    while sites and iteration < max_iterations:
        ...
        for site_name, pos in sorted_sites:
            ...
            if new_seq != seq and mutation_info:  # Only update if mutation was made
                ...
                print(f"    Warning: Could not remove {site_name} site at position {pos} - no suitable silent mutation found")
...
def check_in_frame(seq):
    """Check if sequence is in frame (length divisible by 3)."""
    return len(seq) % 3 == 0


def check_stop_codons(seq):
    """
    Check for stop codons in all three reading frames.
    Returns True if ALL frames have stop codons, False if at least one frame has no stop codons.
    """
    seq = seq.upper()
    ...
    for frame_offset in range(3):
        ...
        for i in range(frame_offset, len(seq) - 2, 3):
            ...
            if len(codon) == 3 and codon in ['TAA', 'TAG', 'TGA']:
                # Check if it's not at the end (allow stop at very end)
                if i + 3 < len(seq) - 3:  # Not in last codon
                    has_stop = True
                    break
        
        frames_with_stops.append(has_stop)
    
    # If any frame has no stop codons, return False (sequence is OK)
    # If all frames have stop codons, return True (sequence should be rejected)
    return all(frames_with_stops)
...
def replace_cdr_in_sequence(seq, cdr_nn, cdr_aa, cdr_type="CDR1"):
    ...
    # Try fuzzy match - find similar sequence
    # This is a simplified approach - in production, would need better alignment
    # For now, if exact match not found, return original
    return seq


def assemble_trb_sequence(row, use_seq_cdrs=False):
    """Assemble TRB sequence from templates and CDR data."""
    # Get gene names
    ...
    if not trbc_seq:
        ...
        return None, []
    if not cdr3_nt or pd.isna(cdr3_nt):
        print(f"    Warning: TRB CDR3 sequence missing")
        return None, []
    
    # Step 1: Align TRBV with CDR3
    trb_seq = align_and_merge_v_cdr3(trbv_seq, cdr3_nt)
    
    # Step 2: Align CDR3 with TRBJ
    trb_seq = align_and_merge_cdr3_j(trb_seq, trbj_seq, cdr3_nt)
    ...
    if use_seq_cdrs and cdr1_nn and cdr2_nn and pd.notna(cdr1_nn) and pd.notna(cdr2_nn):
        # Replace CDRs with sequence CDRs
    ...
    # Step 5: Add TRBC
    trb_seq = align_and_merge_with_c(trb_seq, trbc_seq)
    
    # Step 6: Return sequence and mutations (skip stop codon check for testing)
    return trb_seq, trb_mutations


def assemble_tra_sequence(row, use_seq_cdrs=False):
    """Assemble TRA sequence from templates and CDR data."""
    # Get gene names
    ...
    if not trac_seq:
        ...
        return None, []
    if not cdr3_nt or pd.isna(cdr3_nt):
        print(f"    Warning: TRA CDR3 sequence missing")
        return None, []
    
    # Step 1: Align TRAV with CDR3
    tra_seq = align_and_merge_v_cdr3(trav_seq, cdr3_nt)
    
    # Step 2: Align CDR3 with TRAJ
    tra_seq = align_and_merge_cdr3_j(tra_seq, traj_seq, cdr3_nt)
    ...
    # to find which produces translation ending with "IQNPD"
    best_tra_seq = None
    best_removal = None
    ...
    # Return sequence and mutations (skip stop codon check for testing)
...
def process_tcr(row):
    ...
    trb_imgt_result = assemble_trb_sequence(row, use_seq_cdrs=False)
    trb_seq_result = assemble_trb_sequence(row, use_seq_cdrs=True)
    ...
    tra_imgt_result = assemble_tra_sequence(row, use_seq_cdrs=False)
    tra_seq_result = assemble_tra_sequence(row, use_seq_cdrs=True)
    ...
    # Assemble final TRB-P2A-TRA sequences if both are available
    # TRB_IMGT_CDRs with TRA_IMGT_CDRs
    if trb_imgt and tra_imgt:
        print(f"    Assembling TRB_P2A_TRA_IMGT...")
        ...
        if final_imgt:
            ...
            print(f"    Saved: {tcr_id}/TRB_P2A_TRA_IMGT.fasta")
        else:
            print(f"    Warning: Failed to assemble TRB_P2A_TRA_IMGT")
    else:
        if not trb_imgt:
            print(f"    Warning: TRB_IMGT_CDRs not available for final assembly")
        if not tra_imgt:
            print(f"    Warning: TRA_IMGT_CDRs not available for final assembly")
    
    # TRB_Seq_CDRs with TRA_Seq_CDRs
    if trb_seq and tra_seq:
        print(f"    Assembling TRB_P2A_TRA_Seq...")
        ...
        if final_seq:
            ...
            print(f"    Saved: {tcr_id}/TRB_P2A_TRA_Seq.fasta")
        else:
            print(f"    Warning: Failed to assemble TRB_P2A_TRA_Seq")
    else:
        if not trb_seq:
            print(f"    Warning: TRB_Seq_CDRs not available for final assembly")
        if not tra_seq:
            print(f"    Warning: TRA_Seq_CDRs not available for final assembly")
    
    # Return True if at least one sequence was saved
    return saved_trb or saved_tra
...
def main(input_file=None, output_dir=None, max_tcrs=None):
    ...
    if max_tcrs_limit is not None and max_tcrs_limit > 0:
        ...
        print(f"Processing first {len(df)} TCR entries (MAX_TCRS = {max_tcrs_limit})")
    else:
        print(f"Processing all {len(df)} TCR entries")
